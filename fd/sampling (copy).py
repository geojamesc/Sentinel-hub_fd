...
def prepare_eopatches_paths(sampling_config: SamplingConfig) -> List[str]:
    eopatches_paths = [os.path.join(sampling_config.eopatches_location,'48PTV_1')]# eop_name)
                       #for eop_name in ['48PTV_1']] # _prepare_filesystem(sampling_config).listdir(sampling_config.eopatches_location)]
...
def sample_patch(eop_path: str, sampling_config: SamplingConfig) -> None:
    ...
    #    def __init__(self):
    #        self._listdir = ['input_data/eopatches/48PTV_1', 'input_data/tiffs/48PTV_1']
    #    def get_attrib(self):
    #       return CallableString(self._listdir)
    #    listdir = property(get_attrib)
    ...
    try:
        eop = EOPatch.load(eop_path, lazy_loading=True) # filesystem=filesystem, 
